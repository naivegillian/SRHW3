...
data = Dataset.load_from_df(user_business[["user", "item", "rating"]], reader)
...
import random
all_values = list(range(len(user_business)))
random.seed(123)
random.shuffle(all_values)
all_values=np.array(all_values)
trainset = Dataset.load_from_df(user_business.loc[all_values[:round(len(user_business)*0.8)]][["user", "item", "rating"]], reader)
testset = Dataset.load_from_df(user_business.loc[all_values[round(len(user_business)*0.8):]][["user", "item", "rating"]], reader)
...
dataset = Interactions(user_ids=user_business['user'].values,
                            item_ids=user_business['item'].values,
                            ratings=user_business['rating'].values)
